...
1. **`hello` 握手 + 能力集协商(两级,v3.2)**:宿主首发 `{protocol: 1, base,
   ...
   **npm 发版事实(2026-08-22 复核):官方包已发 npm——`next` dist-tag 指向
   `0.1.1-rc.2`,历史版本全线在册;`latest` 仍指旧版 `0.0.1-rc.1`,引用须
   用精确版本或 `@next`。终态宿主(`host/`)依赖走 npm 精确版本(对齐
   dshSemver),不依赖本地仓检出;实验阶段(M2)的本地源码引用即此路线的
   实验形态。**
