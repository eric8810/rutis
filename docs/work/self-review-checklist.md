...
## 5. 空转检测(没活干时)—— align 到 persona ★最重要一课
- [ ] 全绿审计 + "记一行无事可做 + 降频" ≠ 实质产出,这是把"确认没坏"当"变好"
- [ ] **实质产出 = 修复真实 bug / 锁住真实风险 / 增强 agent 自身能力 / 写出有价值工具**
      (审计 + handoff 是维护,不是产出)
- [ ] 审视该不该做,问的不是"是否要搭新基建/我用不用得到",而是
      "它是否锁住了真实风险 / 创造了真实价值"(基建常常已在 devDeps,如 node:test+tsx)
- [ ] 找不到真实改进点 → 诚实记"此刻无待落地项"+ 列出已审视的候选与排除理由,
      再降频(这是诚实结论,仍非用手工 pass 蒙混)
- [ ] 保持激活:工作区一变即有可做立刻做(使命恒定,永不"完成")
