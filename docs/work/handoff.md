...
## 七十三、技能库沉淀:SKILL-U2 增补 #[ignore] 判别维度(round71/72 实操固化)(自主续跑)

待办转向技能库沉淀。审视发现 SKILL-U2(环境敏感测试判别)有一个真实缺口:
原表述"环境缺就显式红→不改",但 round71/72 实操揭示了必要区分——
- **依赖"缺失的外部产物"(DSH_ROOT/MIN_CORDIS_ROOT checkout)→ 必须 #[ignore]**
  否则 `cargo test -p` 在 clean 环境硬失败、误导人当 bug(host_cordis 曾中招)。
- **依赖"存在产物"(node)→ 默认真跑不 ignore**(llm_e2e/tcp_e2e)。
同仓库须一致,对齐 real_backend 的 `#[ignore="..."]` 模式。

已增补 SKILL-U2(index.md):该区分+判定(看依赖产物默认环境缺还是在)+host_cordis
示例。这是把实操方法论固化,防未来实例重蹈"把该 ignore 的显式红当改 or 当不改"。

**提交**:SKILL-U2 增补。**状态**:工作区净，240 基线绿。
**下一步**:① 技能沉淀可继续(其它 SKILL 待增补) ② 维护。

## 七十四、技能库沉淀:SKILL-E2 增补"反向漂移检查"维度(round70 实操固化)(自主续跑)

延续 round73 技能沉淀。审视 SKILL-E2(文档-代码漂移)发现真实缺口:
原表述只查"记录了但代码还在吗"(单向:文档说做了→代码有吗)。round70 遇到的是
~反向~漂移:文档说"未做/不做/长线可选",但代码早已实现(TS bridge 崩溃测试
f009b72 早用 node:test+tsx landed,handoff 历史却仍写"over-engineering 不做")。
只查单向会漏掉,导致未来实例沿用陈旧"不做"判断。

已增补 SKILL-E2:反向漂移检查 = 对每条历史"长线/不做",git log 该能力有无落地提交;
有 → git 落地 commit + 补 handoff "当前事实标注"纠正误导(不改历史日志)。
TS bridge 为例。与 round73 SKILL-U2 同为实操→方法论的沉淀,防未来实例重蹈。
...
## 七十六、维护:台账 sync + SKILL 计数校准(15 个唯一)(自主续跑)

- **台账 sync**:self_build 推进到 HEAD(1ce5434),entries 15。此前台账 last 落后
  (e96e28e)——因 1ce5434 是文档提交未触发 self_build;已补记,回滚点最新。
- **SKILL 计数校准**:逐条去重核定,技能库是 **15 个唯一 SKILL**
  (E1-E6 + M1-M3 + U1-U2 + W1-W4)。此前多次 handoff/todo 误写"16/17 SKILL"——
  实际 `grep -oE "SKILL-[A-Z][0-9]" | sort -u | wc -l` = 15。已校准,防未来误报。
  注:SKILL-E4 在 index 内因描述自引用被匹配 2 次,故 `grep -c SKILL-` 得 16 是含重复,
  唯一数为 15。

**提交**:cfa2949。**状态**:工作区净,240 基线绿,台账 15 sync HEAD。
**下一步**:维护已收尾;可维持(全绿/健康/持久化)或审视新方向。

## 七十七、自我工具链审视收敛(无代码缺口,诚实结论)(自主续跑)

审视"自我工具健壮性"(距上次真实代码改动已 6 轮),系统性检查后收敛,无需改:
- **skill 工具 list/检索**:实现(driver 级 `tool_result_message`)把 `## ` 后的整个 index
  body 返回;实测 body 含全部 15 SKILL,E4 自引用致 `grep -c` 虚高但唯一数 15。
  之前观察到 list 展示"不完整/折叠"是宿主/展示层截断,非工具 bug。
- **大输出保护**:driver `tool_result_message` 对**所有**工具结果(含 skill)统一
  soft-trim(>1500 字符保头+尾+标记)——round33 的长输出修剪全局生效,skill list
  大返回被保护,不会撑爆上下文。无缺口。
- 结论:自我工具链(list/检索/soft-trim)健康,无真实 bug 待修。

这是审视后的诚实收敛:逐项验证了工具行为,列出为何无需改(非空转 pass)。
本轮无代码改动(连续收敛),与 checklist §5 新纪律一致。
